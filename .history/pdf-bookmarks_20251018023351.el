...
(defun pdf-bookmarks-access ()
  ...
  (if (null pdf-bookmarks-current-file-bookmarks)
    ...
    (let* ((icomplete-was-enabled (bound-and-true-p icomplete-mode))
           (icomplete-vertical-was-enabled (bound-and-true-p icomplete-vertical-mode))
           ...
           ;; Then create the choices from sorted bookmarks
      ...
      (unwind-protect
          (progn
            ;; Enable icomplete modes with settings
            (unless icomplete-was-enabled
              (icomplete-mode 1))
            (unless icomplete-vertical-was-enabled
              (icomplete-vertical-mode 1))
            
            ;; Temporarily set icomplete variables
            (let ((old-show-matches icomplete-show-matches-on-no-input)
                  (old-compute-delay icomplete-compute-delay))
              ...
              (unwind-protect
                  (progn
                    ;; Get the choice
                    (setq choice (completing-read "Go to bookmark: " bookmark-choices nil t))
                    ;; Process the selection
                    (setq selected-bookmark (cdr (assoc choice bookmark-choices))))
                
                ;; Restore icomplete settings
                (setq icomplete-show-matches-on-no-input old-show-matches
                      icomplete-compute-delay old-compute-delay))))
...
(defun pdf-bookmarks-delete ()
  ...
  (if (null pdf-bookmarks-current-file-bookmarks)
    ...
    (let* ((icomplete-was-enabled (bound-and-true-p icomplete-mode))
           (icomplete-vertical-was-enabled (bound-and-true-p icomplete-vertical-mode))
           ;; Sort bookmarks numerically by page number FIRST
           (sorted-bookmarks (sort (copy-sequence pdf-bookmarks-current-file-bookmarks)
                                   (lambda (a b) 
                                     (< (plist-get a :page) 
                                        (plist-get b :page)))))
           ;; Then create the choices from sorted bookmarks
      ...
      (unwind-protect
          (let ((icomplete-show-matches-on-no-input t)
            ...
            ;; Get the choice
            (setq choice (completing-read "Delete bookmark: " bookmark-choices nil t))
    ...
    (let* ((icomplete-was-enabled (bound-and-true-p icomplete-mode))
      ...
      (unwind-protect
          (let ((icomplete-show-matches-on-no-input t)
            ...
            ;; Get the choice
            (setq choice (completing-read "Rename bookmark: " bookmark-choices nil t))
