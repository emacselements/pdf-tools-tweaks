  ...
  (if (null pdf-bookmarks-current-file-bookmarks)
    ...
    (condition-case err
        ...
        (let* ((menu-items
               ...
               (menu-plist 
                (list "Go to Bookmark"
                      (cons "Bookmarks" menu-items)))
               (choice (x-popup-menu t menu-plist)))
          
          (message "Debug: Available menu items: %d" (length menu-items))
          (message "Debug: User selected: %S" choice)
          
          (when choice
            (let ((page (plist-get choice :page))
                  (name (plist-get choice :name)))
              (pdf-view-goto-page page)
              (message "Navigated to bookmark: %s (Page %d)" name page))))
      
      (error
       ;; Fallback to completion buffer
       (message "Popup menu failed, using completion interface...")
       (let* ((bookmark-alist
               (mapcar (lambda (bm)
                         (cons (format "%s (Page %d)" 
                                       (plist-get bm :name)
                               ...
                               bm))
                       pdf-bookmarks-current-file-bookmarks))
              (choice-string (completing-read "Go to bookmark: " bookmark-alist nil t))
              (selected-bookmark (cdr (assoc choice-string bookmark-alist))))
