...
(defun pdf-bookmarks-access ()
  ...
  (if (null pdf-bookmarks-current-file-bookmarks)
    ...
    ;; Simple approach: just use read-string and match
    (let* ((bookmark-list
            (mapcar (lambda (bm)
                      (format "%s (Page %d)" 
                              (plist-get bm :name)
                              (plist-get bm :page)))
                    pdf-bookmarks-current-file-bookmarks))
           (choice (let ((minibuffer-local-completion-map
                          (copy-keymap minibuffer-local-completion-map)))
                     (define-key minibuffer-local-completion-map " " 'self-insert-command)
                     (completing-read "Go to bookmark: " bookmark-list))))
      
      (message "Debug: Available choices: %S" bookmark-list)
