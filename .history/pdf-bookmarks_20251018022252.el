...
(defun pdf-bookmarks-rename ()
  ...
  (if (null pdf-bookmarks-current-file-bookmarks)
    ...
    (let ((fido-was-enabled fido-vertical-mode)
      ...
      ;; Try to get the choice, ensuring cleanup happens
      (condition-case nil
          (setq choice (completing-read "Rename bookmark: " 
                                       (mapcar (lambda (bm)
                                                 (cons (format "%s (Page %d)" 
                                                               (plist-get bm :name)
                                                               (plist-get bm :page))
                                       ...
                                       nil t))
        (quit 
         ;; Turn off fido-mode if we enabled it, then re-signal quit
         (unless fido-was-enabled
           (fido-vertical-mode -1))
         (signal 'quit nil)))
      
      ;; Turn off fido-mode if we enabled it (for successful completion)
      (unless fido-was-enabled
        (fido-vertical-mode -1))
      
      ;; Process the selection
      (setq selected-bookmark (cdr (assoc choice (mapcar (lambda (bm)
                                                           (cons (format "%s (Page %d)" 
                                                                         (plist-get bm :name)
                                                                         (plist-get bm :page))
                                                                 bm))
