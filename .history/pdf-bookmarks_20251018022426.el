...
(defun pdf-bookmarks-delete ()
  ...
  (if (null pdf-bookmarks-current-file-bookmarks)
    ...
    (let ((fido-was-enabled fido-vertical-mode)
          choice
          selected-bookmark
          (bookmark-choices (mapcar (lambda (bm)
                                     (cons (format "%s (Page %d)" 
      ...
      (unwind-protect
          (progn
            ;; Enable fido-mode if it wasn't already on
            (unless fido-was-enabled
        ...
        ;; ALWAYS turn off fido-mode if we enabled it, even on quit
        (unless fido-was-enabled
          (fido-vertical-mode -1)))
