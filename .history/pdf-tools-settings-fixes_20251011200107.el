...
(defun pdf-bookmarks-access ()
  ...
  ;; Skip loading, just use hardcoded test data for now
  (let* ((test-bookmarks '((:name "Page 1" :page 1)
                          (:name "Page 2" :page 2) 
                          (:name "Page 3" :page 3)))
         (bookmark-alist
          (mapcar (lambda (bm)
      ...
      (let ((selected-bookmark (cdr (assoc choice-string bookmark-alist))))
        ...
        (if selected-bookmark
            (let ((page (plist-get selected-bookmark :page)))
              (message "Debug: Going to page %d" page)
              (pdf-view-goto-page page)
              (message "Navigated to page %d" page))
          (message "Error: Could not find bookmark"))))))
