...
(defun pdf-bookmarks-access ()
  ...
  (interactive)
  (message "DEBUG: pdf-bookmarks-access function called!")
  ...
  (message "Debug: Found %d bookmarks" (length pdf-bookmarks-current-file-bookmarks))
  
  (if (null pdf-bookmarks-current-file-bookmarks)
      (message "No bookmarks found for this PDF")
    
    (message "DEBUG: About to show completion menu...")
    ;; Simple minibuffer completion
    (let* ((bookmark-alist
            (mapcar (lambda (bm)
      ...
      (message "Debug: Bookmark alist: %S" bookmark-alist)
      
      ;; Force minibuffer completion
      (let ((choice-string (completing-read 
                           "Go to bookmark (use TAB to complete): " 
                           bookmark-alist 
                           nil    ; predicate 
                           t      ; require-match
                           nil    ; initial-input
        ...
        (message "Debug: User selected string: '%s'" choice-string)
        
        (let ((selected-bookmark (cdr (assoc choice-string bookmark-alist))))
          (message "Debug: Looking for choice-string: '%s'" choice-string)
          (message "Debug: Found selected-bookmark: %S" selected-bookmark)
          
          (if selected-bookmark
              (let ((page (plist-get selected-bookmark :page))
                    (name (plist-get selected-bookmark :name)))
                (message "Debug: Extracted page=%S (type: %s), name=%S" 
                         page (type-of page) name)
                (if (and page (numberp page) (> page 0))
                    (progn
                      (message "Debug: About to call pdf-view-goto-page with page %d" page)
                      (message "Debug: Current buffer: %s" (current-buffer))
                      (message "Debug: Current major mode: %s" major-mode)
                      
                      ;; Add error handling around pdf-view-goto-page
                      (condition-case err
                          (pdf-view-goto-page page)
                        (error
                         (message "Error in pdf-view-goto-page: %s" (error-message-string err))
                         (message "Error details: %S" err)
                         (message "Stack trace:")
                         (backtrace)))
                      
                      (message "Navigated to bookmark: %s (Page %d)" name page))
                  (message "Error: Invalid page number: %S" page)))
            (message "Error: Could not find bookmark for selection: '%s'" choice-string)))))))
